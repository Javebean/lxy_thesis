...
var delete_but = function(source){
	$(source).click(function(){
		...
		deleteItem("lxy/deletea",de_id);
	});
}


var loadMessages = function(start){
	$.getJSON("lxy/getteabypage/"+start+"/"+items,function(data){
		...
		$.each(data,function(){
			var html = '<tr>'
						...
						+'<td>'+this.position+'</td>'
						+'<td>'
							+'<a type="button" class="btn btn-primary" href="detail_tea.jsp?tea_id='+this.id+'">详细</a>'
							+'<a type="button" class="btn btn-success" href="edit_tea.jsp?tea_id='+this.id+'">编辑</a>'
	 ...
	 $(function(){
		 loadMessages(1);
		 pagebutton("lxy/getteacount",items);
