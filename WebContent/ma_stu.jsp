...
var delete_but = function(source){
	$(source).click(function(){
		...
		deleteItem("lxy/delestu",de_id);
	});
}


var loadMessages = function(start){
	$.getJSON("lxy/getstubypage/"+start+"/"+items,function(data){
		...
		$.each(data,function(){
			var html = '<tr>'
						...
						+'<td>'+this.email+'</td>'
						+'<td>'
							+'<a type="button" href="detail_stu.jsp?stu_id='+this.id+'" class="btn btn-primary">详细</a>'
							+'<a type="button" href="edit_stu.jsp?stu_id='+this.id+'" class="btn btn-success">编辑</a>'
	 ...
	 $(function(){
		 loadMessages(1);
		 pagebutton("lxy/getstucount",items);
